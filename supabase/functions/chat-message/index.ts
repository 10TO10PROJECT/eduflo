...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (session.turn_count >= SESSION_TURN_LIMIT)     return errResp(429, "SESSION_LIMIT");
    if (new Date(session.expires_at) < new Date())    return errResp(410, "SESSION_EXPIRED");
    if (session.total_cost_krw >= SESSION_COST_CAP_KRW) return errResp(429, "BUDGET_EXCEEDED");
    ...
    if (userMsgErr) console.error("user turn insert error:", userMsgErr.message);
  ...
  return rows.map(r => {
    ...
    for (const b of r.content_blocks as any[]) {
      if (b.type === "text") {
        ...
        const names = (b.items as any[]).map((a: any) => `${a.name}(id:${a.id})`).join(", ");
...
function needsAcademyRequery(text: string): boolean {
  const triggers = ["다른", "추가", "더", "비슷한", "저렴한", "가까운", "근처", "재추천", "다시"];
